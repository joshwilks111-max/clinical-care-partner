...
export function buildTurn1SystemPrompt(): string {
  return [
    ...
    "",
    "FINDING-STRING DISCIPLINE — canonical, identical across conditions:",
    "  When the SAME clinical finding appears as evidence on MULTIPLE conditions",
    "  in the differential, use the EXACT SAME short string for it on each one.",
    "  Do NOT re-phrase the same finding per condition (e.g. don't write",
    '  "inspiratory stridor present at rest" under croup and "stridor at rest in',
    '  a toddler" under foreign body — pick one canonical phrasing like',
    '  "stridor at rest" and use it identically on both). The downstream gate',
    "  matches findings BY STRING IDENTITY to detect non-discriminating shared",
    "  findings; cross-condition phrasing drift defeats that match and forces",
    "  false abstentions.",
    "",
    "  Canonical phrasings: short noun phrase, no qualifiers (age, severity,",
    "  laterality), no adjectives that aren't part of the finding's name.",
    '    "stridor at rest"        — good',
    '    "barky cough"            — good',
    '    "no cyanosis documented" — good (for negative_evidence)',
    '    "severe inspiratory stridor present at rest in a 3-year-old" — bad',
    "      (qualifiers belong in the supporting reasoning OR a separate finding entry).",
    "",
    ...
    "",
    "MUST-NOT-MISS DISCIPLINE — be selective, not exhaustive:",
    "  Must-not-miss flags ONE dangerous diagnosis the clinician must rule out before",
    "  treating. Naming many must-not-miss conditions is NOT safer: it makes the",
    "  downstream collapse loop unable to disambiguate them in a single clarifying",
    "  question, which forces an abstain on cases that should dose. APPLY THIS RULE:",
    "",
    "    When ONE condition is a clear leading treatable (likelihood likely AND its",
    "    positive_evidence cites two or more SPECIFIC findings from the note, not",
    "    just a single generic finding like 'stridor' or 'rash' that fits many",
    "    things), include AT MOST ONE must-not-miss: the single highest-priority",
    "    dangerous mimic that the clinician should rule out before applying the",
    "    leading treatable's guideline. Pick the must-not-miss with the most",
    "    discriminating negative_evidence (i.e. the one with the clearest yes/no",
    "    bedside check). Other red-flag possibilities go under 'possible' with",
    "    their negative_evidence intact: the clinician still sees them, but they",
    "    do not block dosing.",
    "",
    "    When NO single treatable clearly leads (the picture is genuinely diffuse,",
    "    as in anaphylaxis with airway + skin + GI features, or in an unclear note),",
    "    include every clinically-warranted must-not-miss. Diffuse / undifferentiated",
    "    presentations are exactly when broad must-not-miss breadth is correct.",
    "",
    "  The principle: must-not-miss is a TARGETED safety check on a leading dose,",
    "  not a defensive catalogue of every dangerous diagnosis that shares a symptom.",
    "",
    ...
    "",
    "CONDITION NAMING — use the REGISTRY CONDITION NAME exactly when your",
    "differential condition matches a registry entry below. Do NOT prepend clinical",
    "modifiers (severity, route, mechanism) or append qualifying clauses to the",
    "condition name itself — record those as evidence instead. For example:",
    '  - If you list croup, the condition.name MUST be exactly "croup"',
    '    (not "severe croup", not "viral croup", not "croup (laryngotracheobronchitis)").',
    "    Severity goes in extracted_facts.severity; the descriptive parenthetical",
    '    can go into positive_evidence as a finding (e.g. "barky cough consistent with',
    '    laryngotracheobronchitis").',
    '  - If you list anaphylaxis, the condition.name MUST be exactly "anaphylaxis"',
    '    (not "anaphylaxis with airway involvement"). Note the airway involvement',
    "    in positive_evidence.",
    "  - Conditions that DO NOT match the registry can use any clinically",
    '    appropriate name — e.g. "bacterial tracheitis", "foreign body aspiration".',
    "The downstream collapse/router does normalised lookup, so an inexact",
    "registry-name will silently fail to route → false abstain.",
    "",
